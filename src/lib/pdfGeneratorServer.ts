...
// Server-side PDF generation for quotations
export async function generateQuotationPdfBuffer(quotation: any): Promise<Uint8Array> {
  const doc = new jsPDF();
  const pageHeight = doc.internal.pageSize.height || doc.internal.pageSize.getHeight();
  const pageWidth = doc.internal.pageSize.width || doc.internal.pageSize.getWidth();
  let currentY = 15;
  ...
  // S.NO and Date on right side
  doc.setFontSize(10);
  const serialNumber = quotation.serialNo;
  doc.text('S.NO :', pageWidth - 60, currentY);
  doc.text(serialNumber, pageWidth - 45, currentY);
  doc.text('Date :', pageWidth - 60, currentY + 15);
  doc.text(quotation.date ? new Date(quotation.date).toLocaleDateString('en-GB') : '', pageWidth - 45, currentY + 15);

  // Patient Details
  doc.setFont('helvetica', 'normal');
  doc.text('To :', 15, currentY);
  doc.text(quotation.patientName || '', 35, currentY);
  currentY += 6;
  doc.text('Address :', 15, currentY);
  doc.text(quotation.patientAddress || '', 35, currentY);
  currentY += 6;
  doc.text('Contact :', 15, currentY);
  doc.text(quotation.patientContact || '', 35, currentY);
  currentY += 6;
  doc.text('Age :', 15, currentY);
  doc.text(quotation.patientAge ? String(quotation.patientAge) : '', 35, currentY);
  currentY += 6;
  doc.text('Sex :', 15, currentY);
  doc.text(quotation.patientGender || '', 35, currentY);
  currentY += 8;
  ...
  const itemRows = (quotation.items || []).map((item: any, idx: number) => {
    ...
    if (item.isPriceInclGst) {
      ...
      amount = ((item.price || 0) * item.quantity).toFixed(2);
    }
    // Use item.date if present, otherwise use quotation.date
    const rowDate = item.date ? new Date(item.date).toLocaleDateString('en-GB') : (quotation.date ? new Date(quotation.date).toLocaleDateString('en-GB') : '');
    return [
      rowDate,
      (item.srNo || idx + 1).toString(),
      `${item.type || ''}${descriptions ? '\n' + descriptions : ''}`,
  ...
  const cgstAmount = quotation.cgstAmount || 0;
  const sgstAmount = quotation.sgstAmount || 0;
  ...
  autoTable(doc, {
    ...
    margin: { left: 15, right: 20, bottom: 100 },
    tableLineWidth: 0.1,
    tableLineColor: [0, 0, 0]
  });
